...
export function PhoneConnectButton() {
  ...
  // Render the QR whenever the address changes. Generated in the browser — the URL never leaves
  // this machine, which is rather the point of the whole feature.
  useEffect(() => {
  ...
  if (!data?.available || !user?.is_admin) return null;
          ...
          <div className="max-h-[90dvh] w-full max-w-sm overflow-y-auto rounded-2xl bg-white p-5 shadow-xl dark:bg-zinc-900" onClick={e => e.stopPropagation()}>
              ...
              <>
                ...
                {state === 'auth' && (
                  <>
                    ...
                    {data?.authUrl && (
                      // Escape hatch: a sign-in provider can refuse an embedded window at any time,
                      // and then the in-app login is a dead end with no way out.
                      <a
            ...
            {state === 'needs_funnel' && (
              <>
                ...
                <a
                  href={data?.helpUrl || 'https://login.tailscale.com/admin/settings/features'} target="_blank" rel="noopener noreferrer"
