...
export function meRoutes(app: FastifyInstance): void {
  app.patch('/api/me', async (req, reply) => {
    ...
    const { prefers_dark, preferred_lang, password, old_password, has_seen_tour, pinned_chains, emoji } = (req.body ?? {}) as {
      prefers_dark?: boolean; preferred_lang?: string; password?: string; old_password?: string; has_seen_tour?: boolean; pinned_chains?: string[]; emoji?: string | null;
    };
